...
class RadarMapDisplay(RadarDisplay):
    ...
    def plot_ppi_map(
        ...
        if type(basemap) != Basemap:
            ...
            if using_corners:
                ...
                basemap = Basemap(
                    ...
                    resolution=resolution, ax=ax, epsg=3857, **kwargs)
        ...
        pm = basemap.pcolormesh(
            ...
            vmin=vmin, vmax=vmax, cmap=cmap, norm=norm, alpha=0.5)

        if shapefile is not None:
            basemap.readshapefile(shapefile, 'shapefile', ax=ax)

        basemap.arcgisimage(service='World_Shaded_Relief', xpixels = 800, dpi=400, verbose= True)
        #basemap.shadedrelief(ax=ax, scale=0.5)
